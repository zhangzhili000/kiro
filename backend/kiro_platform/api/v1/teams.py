...
from kiro_platform.models.team import Team, TeamMember, TeamDocument
from kiro_knbase.models.document import Document
from kiro_platform.schemas.team import (
...
def list_team_users(
    ...
    team_member_ids = db.query(TeamMember.user_id).filter(TeamMember.team_id.in_(team_ids)).all()
    user_ids = [tm[0] for tm in team_member_ids]
    
    users = db.query(User).filter(User.id.in_(user_ids), User.is_active == True).all()
    
    return [
        {"id": user.id, "username": user.username, "name": user.name or user.username}
        for user in users
    ]
